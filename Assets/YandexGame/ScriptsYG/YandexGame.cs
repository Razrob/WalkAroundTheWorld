...
namespace YG
{
    ...
    public class YandexGame : MonoBehaviour
    {
        ...
        public static void SaveEditor()
        {
            ...
            PlayerPrefs.SetString("GAME_PROGRESS_SAVE_KEY", json);

        }

        public static void LoadEditor()
        {
            if (PlayerPrefs.HasKey("GAME_PROGRESS_SAVE_KEY"))
                savesData = JsonUtility.FromJson<SavesYG>(PlayerPrefs.GetString("GAME_PROGRESS_SAVE_KEY"));
            else
                savesData= new SavesYG();
        ...
        static void AfterLoading()
        {
            ...
            InvokeAfterInitActions();
        }


        private static List<Action> _afterInitActions = new List<Action>();
        ...
        private static void InvokeAfterInitActions()
        {
            Debug.Log("InvokeAfterInitActions");
            for (int i = _afterInitActions.Count - 1; i >= 0; i--)
            {
                _afterInitActions[i]();
                _afterInitActions.RemoveAt(i);
            }
        }
        ...
        public void _SaveProgress()
        {
            if (_SDKEnabled)
            {
                ...
                if (infoYG.saveCloud && timerSaveCloud >= infoYG.saveCloudInterval - 0.01f)
                {
                    timerSaveCloud = 0;
                    SaveCloud();
                }
...
//        }
        ...
        {
            Debug.Log("SetLanguage1: " + language);
            string lang = "en";
